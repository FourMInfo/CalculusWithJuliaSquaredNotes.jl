...
# `conventional_latex` typesets one value at a time: `2π/3` rather than `\frac{2}{3} ~ \pi`,
# and no leading space before a minus sign, which would stop Pandoc from opening the math.
typeset(v) = LaTeXString(conventional_latex(v))
latexify(DataFrame(θ = typeset.(θs),
                   x = typeset.(exact_trig_values.(cos.(θs))),
                   y = typeset.(exact_trig_values.(sin.(θs)))))
```
