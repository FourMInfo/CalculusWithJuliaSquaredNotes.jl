...
# `Symbolics` canonicalises -√2/2 to the product (-1//2)*√2, which Latexify writes as
# `$ - \frac{1}{2} ~ \sqrt{2}$` -- with a space after the opening `$`. Pandoc does not
# open inline math on `$ `, so that cell would render as a literal dollar sign instead
# of mathematics. Close the gap; every other cell is already fine.
Markdown.parse(replace(sprint(print, tbl), "\$ -" => "\$-"))
```
