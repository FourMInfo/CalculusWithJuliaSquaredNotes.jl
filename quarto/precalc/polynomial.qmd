...
Here we discuss some vocabulary and basic facts related to polynomials and show how the `Symbolics` package can be used to model polynomial expressions. `Symbolics` provides a Computer Algebra System (CAS) written in `Julia` itself; it comes with `CalculusWithJuliaSquared`, which re-exports it. Later we will discuss the `Polynomials` package for polynomials.
...
Symbolic math programs include well-known ones like the commercial programs Mathematica and Maple. Mathematica powers the popular [WolframAlpha](https://www.wolframalpha.com) website, which turns "natural" language into the specifics of a programming language. The open-source [Sage](https://www.sagemath.org/) project is an alternative to these two commercial giants. It includes a wide-range of open-source math projects available within its umbrella framework. (`Julia` can even be run from within the free service [CoCalc](https://cocalc.ai), which began as the Sage project's cloud service.) A more focused project for symbolic math, is the [SymPy](https://www.sympy.org) Python library. SymPy is also used within Sage. However, SymPy provides a self-contained library that can be used standalone within a Python session.

The [Symbolics](https://github.com/JuliaSymbolics/Symbolics.jl) package for `Julia` provides a "fast and modern CAS for fast and modern language." It is written entirely in `Julia`, and it is the CAS these notes use. The symbolic functions used across the notes are listed in the [Julia overview](julia_overview.qmd#symbolic-math).

---


To work symbolically, we create symbolic objects to be our indeterminate symbols. The `@variables` macro does this:


```{julia}
@variables a b c x zs[1:10];
```

The above shows that multiple symbols can be defined at once. (The trailing `;` only hides the cell's output, a list of the new variables.) `Symbolics` treats a variable as a *real* number unless told otherwise, which is what we want for polynomials with real coefficients. The `zs[1:10]` notation creates a container of $10$ different symbols, `zs[1]` through `zs[10]`.  The *macro* `@variables` does not need assignment, as the variable(s) are created behind the scenes by the macro.
...
`Symbolics` overloads many `Julia` functions to work seamlessly with symbolic expressions, which makes working with polynomials quite natural. To illustrate, using the just defined `x`, here is how we can create the polynomial $-16x^2 + 100$:
...
The `typeof` function shows that `p` is of a symbolic type (`Num`):
...
`Symbolics` provides the `substitute` function for this.
...
```{julia}
substitute(p, x => (x-1)^2)
```

After the expression comes a pair (designated by `a=>b`) where the left-hand side is the variable to substitute for, and the right-hand side the new value. (This mirrors a similar use with `replace`.) To substitute for several variables at once, pass a dictionary of such pairs, `Dict(x => 2, a => 3)`, as in the [Julia overview](julia_overview.qmd#symbolic-math). The value to substitute can depend on the variable, as illustrated; be a different variable; or be a numeric value, such as $2$:
...
There is no shorter way to write this. A symbolic expression is not a function, so calling it like one, `p(4)`, is an error, even when `x` is its only variable. (The error message suggests `@syms`, which is for declaring an unknown *function*, such as $f(x)$, symbolically. It does not make `p` callable.) Keeping `substitute` for expressions and `f(x)` for functions keeps the distinction between the two in plain view.
...
The value, $36$ is still symbolic, but clearly an integer. If we are just looking at the output, we can easily translate from the symbolic value to an integer, as they print similarly. However the conversion to an integer, or another type of number, does not happen automatically. Taking a square root shows the difference: `Julia` would compute the square root of the integer $36$ as $6.0$, but the symbolic $36$ just gets a symbolic square root:

```{julia}
sqrt(y)
```

If a number is needed to pass along to another `Julia` function, it needs to be converted. Beware that the generic `float` method, which converts most numbers to floating point, does *not* do it here: `float(y)` quietly returns the same symbolic $36$, as its type shows:

```{julia}
typeof(float(y))
```

The conversion is done by `Symbolics.value`, which returns the number the symbolic object holds:

```{julia}
Symbolics.value(y), sqrt(Symbolics.value(y))
```

`Symbolics.value` returns the number in the type it was stored as, so its output type varies: an integer here, but a rational number when a rational was substituted:

```{julia}
Symbolics.value(substitute(p, x => 1//2))
```

When free symbols remain, as in `substitute(a*x^2 + b, x => 2)`, there is no number to return, and `Symbolics.value` hands back the expression rather than failing. So when a number is required, check for one: `Symbolics.value(...) isa Number` is `false` in that case.
...
```{julia}
p = 200 - 16x^2
Symbolics.value(substitute(p, x => 2))
```
...
The `build_function` function turns a symbolic expression into a `Julia` function. It is given the expression and then the variable (or variables) that become the function's arguments; `expression = Val(false)` asks for a ready-to-call function rather than the `Julia` code for one:


```{julia}
#| hold: true
pp = build_function(p, x; expression = Val(false))
pp(2)
```

The use above is straightforward. Only slightly more complicated is the use when there are multiple symbolic values. For example:


```{julia}
#| hold: true
p = a*x^2 + b
pp = build_function(p, a, b, x; expression = Val(false))
pp(1,2,3)
```

This evaluation matches `a` with `1`, `b` with `2`, and `x` with `3`, as that is the order the variables were listed in. There is no default order to guess: the arguments are always the variables you list, in the order you list them. Listing them differently changes the function:


```{julia}
#| hold: true
p = a*x^2 + b
pp = build_function(p, x, a, b; expression = Val(false))
pp(1,2,3) # computes 2*1^2 + 3
```

(To find which variables an expression contains, `Symbolics.get_variables(p)` lists them. For a single evaluation with more than one variable, `substitute` with a dictionary of pairs makes the matching explicit.)
...
(Plotting symbolic expressions with `Plots` is similar to plotting a function, in that the expression is passed in as the first argument. This works because `Symbolics` defines a `Plots` "recipe" for its expressions. The expression must have only one free variable, as above. Be careful here: at the time of writing, an expression with more than one, such as `plot(a*x^2, -1, 1)`, does not stop with an error message. It crashes the `Julia` session itself (in a notebook, the kernel dies). To plot such an expression, substitute values for the other variables first: `plot(substitute(a*x^2, a => 2), -1, 1)`.)
...
let
    ...
    plot!(x -> x^5,  -3,3, legend=false, xticks=false, yticks=false, subplot=2, title=L"$n = $ odd, $a_n > 0$")
...
p = a*x^2 + b*x + c
n = 2    # the degree of p
q = expand(x^n * substitute(p, x => 1/x))
```

The term $a x^2/x^2$ is left as it is: `expand` multiplies out, but does not cancel common factors. `simplify_fractions` does:

```{julia}
simplify_fractions(q)
```
...
This observation is the start of [Descartes' rule of signs](https://en.wikipedia.org/wiki/Descartes%27_rule_of_signs), which counts the change of signs of the coefficients in `p` to say something about how many possible crossings there are of the $x$ axis by the graph of the polynomial $p$. (Stewart Levin's short note ["Descartes' Rule of Signs - How hard can it be?"](https://web.archive.org/web/20250811235456/https://sepwww.stanford.edu/oldsep/stew/descartes.pdf) builds the rule from this same observation, that each power of $x$ dominates in its own region.)
...
The `expand` function will perform these algebraic manipulations without fuss:
...
Factoring a polynomial is several weeks worth of lessons, as there is no one-size-fits-all algorithm available to teach to algebra students. There are some tricks that are taught: for example factoring differences of perfect squares, completing the square, the rational root theorem, $\dots$. But in general the solution is not automated without some more advanced techniques. The `factored_poly` function will find all rational factors (terms like $(qx-p)$), but will leave terms that do not have rational factors alone. It is given the polynomial and its variable. For example:


```{julia}
factored_poly(x^3 - 6x^2 + 11x - 6, x)
```
...
```{julia}
factored_poly(x^5 - 5x^4 + 8x^3 - 8x^2 + 7x - 3, x)
```
...
Our definition of a polynomial is in terms of algebraic expressions which are easily represented by symbolic objects, but not objects from base `Julia`. (Later we discuss the `Polynomials` package for representing polynomials. There is also the `AbstractAlgebra` package for a more algebraic treatment of polynomials.)
...
This is easy---but can also be confusing. The function object is `f`, the expression is `f(x)`---the function evaluated on a symbolic object. The two are evaluated differently: the function by calling it, `f(2)`, and the expression by substitution:
...
For some uses, the distinction is unnecessary to make, as some functions accept either one. For `Plots` there is a recipe – either `plot(f, a, b)` or `plot(f(x), a, b)` will produce the same plot using the `Plots` package.
...
#| hold: true
#| echo: false
choices = [q"p*2", q"p(2)", q"p[2]", q"substitute(p, x => 2)"]
answ = 4
radioq(choices, answ)
```
...
Use `factored_poly` to factor the polynomial $x^{11} - x$. How many factors are found?
...
Use `factored_poly` to factor the polynomial $x^{12} - 1$. How many factors are found?
...
Use `simplify_fractions` to simplify the expression `((x-h)^3 - x^3) / h` where `x` and `h` are symbolic constants. What is the value:
