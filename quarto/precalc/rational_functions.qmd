...
`CalculusWithJuliaSquared` brings the `Symbolics` computer algebra system and `Plots` with it. The `Polynomials` package can be loaded in full alongside them: none of the names it exports is already taken. After loading it, `variable`, `degree`, `roots`, `derivative` and `integrate` refer to its functions. (`Symbolics` has its own `variable` and `derivative`, but does not export them, so they do not clash; they are written `Symbolics.variable` and `Symbolics.derivative`.)
...
Formally, an [asymptote](https://en.wikipedia.org/wiki/Asymptote) of a curve is a line such that the distance between the curve and the line approaches $0$ as they tend to infinity. Tending to infinity can happen as $x \rightarrow \pm \infty$ *or* $y \rightarrow \pm \infty$, the former being related to *horizontal asymptotes* or *slant asymptotes*, the latter being related to *vertical asymptotes*.
...
```{julia}
@variables x   # a Symbolics variable is real by default
num = (x-1)^2 * (x-2)
den = (x+3) * (x-3)
```
...
A similar result can be found using the `partial_fractions` function (`SymPy` calls it `apart`), which can be easier to use if the expression is not given in terms of a separate numerator and denominator. It needs to be told which symbol is the variable:


```{julia}
g(x) = (x-1)^2 * (x-2) / ((x+3)*(x-3))  # as a function
h = g(x)                                # a symbolic expression
partial_fractions(h, x)
```

This decomposition breaks the rational expression into two pieces: $x-4$ and $\frac{40}{3(x+3)} + \frac{2}{3(x-3)}$. The first piece would have a graph that is the line with slope $1$ and $y$-intercept $-4$. As $x$ goes to $\infty$, the second piece will clearly go towards $0,$ as this simple graph shows:


```{julia}
plot(partial_fractions(h, x) - (x - 4), 10, 100)
```
...
The factored form, as $h$ is presented, is a bit hard to work with, rather we use the expanded form, which we get through the `expand` function. On a quotient it multiplies out the numerator and the denominator separately:
...
(`SymPy`'s `cancel` would also cancel any factor common to the top and bottom. `expand` never does; that is the job of `simplify_fractions`. Here there is no common factor, so the two agree.)
...
quo, r = divrem(numerator(p), denominator(p))  # or partial_fractions(p, x)
quo
```
...
top = 5x^3 + 6x^2 +2
bottom = x-1
quo, r = divrem(top, bottom)
```

(We name the remainder `r`, not `rem`: assigning to `rem` would hide `Julia`'s `rem` function, the integer remainder, for the rest of the session.)
...
where both $r(c)$ and $s(c)$ are non zero. Knowing $m$ and $n$ (the multiplicities of the root $c$) allows the following to be said:
  ...
  * If $m \geq n$ then $x=c$ will not be a vertical asymptote. (The value $c$ will be known as a removable singularity). In this case, the graph of $p(x)/q(x)$ and the graph of $(x-c)^{m-n}r(x)/s(x)$ will differ, though very slightly, as the latter will include a value for $x=c$, whereas $x=c$ is not in the domain of $p(x)/q(x)$.
...
This isn't ideal, as the large values are still computed, just the viewing window is clipped. This leaves the vertical asymptotes still affecting the graph.
...
```{julia}
plot(rangeclamp(f, 30), -25, 25)  # rangeclamp is in the CalculusWithJuliaSquared package
```
...
With the computer, where it is convenient to draw a graph, it might be better to emphasize the sign on the graph of the function, but at times numeric values are preferred. The `sign_chart` function from `CalculusWithJuliaSquared` does this analysis by numerically identifying points where the function is $0$ or $\infty$ and indicating the sign as $x$ crosses over these points.
...
This format is a bit different from above, but shows to the left of $-1$ a minus sign; between $-1$ and $0$ a plus sign; between $0$ and $1$ a minus sign; and between $1$ and $3/2$ a plus sign.
...
The `Polynomials` package, loaded at the top of this section, has support for rational functions. The `//` operator can be used to create rational expressions from polynomial expressions. (Note that the first line below makes `x` name the `Polynomials` variable from here on, in place of the symbolic `x` used so far.)
...
For polynomials as simple as these, this computation is not a problem, but there is the very real possibility that the lowest term computation may be incorrect. The `lowest_terms` function uses a numeric algorithm, as the not-quite-integer coefficients above show, and does not, as would be done by hand, factor the polynomials and then cancel common factors.

`Symbolics` does the exact version. A `Polynomials` polynomial can be evaluated at a symbolic variable, which turns it into a symbolic expression; `simplify_fractions` then cancels the common factor exactly:


```{julia}
@variables t   # a new symbol, as `x` now names the Polynomials variable
simplify_fractions(p(t) / q(t))
```
...
This observation, along with a detailed analysis provided by [Kobel, Rouillier, and Sagraloff](https://dl.acm.org/doi/10.1145/2930889.2930937) ([open copy on arXiv](https://arxiv.org/abs/1605.00410)) provides a means to find intervals that enclose the real roots of a polynomial.
...
For square-free polynomials, the `RealPolynomialRoots` package implements a basic version of the  paper of [Kobel, Rouillier, and Sagraloff](https://dl.acm.org/doi/10.1145/2930889.2930937) ([open copy on arXiv](https://arxiv.org/abs/1605.00410)) to identify the real roots of a polynomial using the Descartes rule of signs and the Möbius transformations just described.
...
The `roots` function from the `Polynomials` package, which works in floating point, gets the cluster wrong: it reports the two clustered roots as a pair of complex numbers.


```{julia}
filter(z -> abs(z) < 0.1, roots(u))   # the roots near 0.0079
```

The `root_enclosures` function from `CalculusWithJuliaSquared` settles the question. It works from the exact polynomial, so we first evaluate `u` at the symbol `t`, which gives the same polynomial as a symbolic expression. Each real root comes back inside an interval that is guaranteed to contain it, printed as `[midpoint +/- radius]`:


```{julia}
es = root_enclosures(u(t), t)
```

Two intervals that do not overlap can only hold two different roots, so this shows there are three distinct real roots. The two in the cluster differ by only about $2.6 \cdot 10^{-18}$:


```{julia}
Nemo.overlaps(es[1], es[2])
```

(`Nemo`, which does the exact computation, comes with `CalculusWithJuliaSquared` but must be written out in front of its functions.) For plain floating-point values, `numeric_roots` with `real_only = true` also returns three real roots, but the two clustered ones differ only in the last digit. Floating-point numbers alone cannot prove that these are two roots rather than one root computed twice:


```{julia}
numeric_roots(u(t), t; real_only = true)
```

Though the implementation in `RealPolynomialRoots` doesn't handle such large polynomials, the authors of the algorithm have implementations that can quickly solve polynomials with degrees as high as $10,000$.
