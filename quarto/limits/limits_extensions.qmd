...
#| echo: false
#| output: false
# a partial sum of a series is written in ascending powers; reset after the next cell
...
The value is right, and the route says how it was reached: `:squeeze` *bounds* the
expression with interval arithmetic and reports the midpoint of a box that has closed on
$2/3$ — a float, because a float is all a bound knows. Nothing here was *derived*. Rewriting
$a^m$ as $e^{m\log a}$ — the same move used twice in the previous chapter — puts the
expression in log-exponential form, which is the domain the Gruntz engine is defined on.
That rewrite is scaffolding for the engine, not mathematics (the two forms are the same
function); what changes mathematically is that the exact route now applies, and the answer
comes back derived:
