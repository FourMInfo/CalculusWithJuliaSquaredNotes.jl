...
as of v1.1.5, it does not cancel common factors, so `limit((x^2-1)/(x-1), x, 1)` returns
...
The first three are exact and give rationals. `symlim` also cross-checks its answer against
a numeric probe and warns if the two disagree.
:::
...
giving $\log(x)$, by the `:gruntz` route. Rewriting an expression into the form a method
expects is a normal part of symbolic work, not a workaround.
