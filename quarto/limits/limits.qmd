...
import SymbolicLimits            # symbolic limits: a companion package to Symbolics
import IntervalArithmetic        # rigorous enclosures, for the squeeze theorem
plotly()
```

`CalculusWithJuliaSquared` re-exports `Plots` and `Symbolics`, so neither needs a `using` of
its own. `SymbolicLimits` is imported rather than `using`-ed so that its `limit` does not
collide with other names; the helper defined below wraps it.
...
`Symbolics` does not compute limits itself. The `symlim` function of
`CalculusWithJuliaSquared` fills that gap: it takes the expression, the variable and a
value for $c$ -- the same three things in the notation $\lim_{x \rightarrow c}f(x)$ -- and
returns a pair, the limit together with the *route* by which it was found.


For example, the limit at $0$ of $(1-\cos(x))/x^2$:


```{julia}
@variables x::Real
symlim((1 - cos(x)) / x^2, x, 0)
```

The answer is the exact rational $1/2$, and `:series` records how it was obtained. Knowing
the route matters here in a way it does not with a black box, because the routes have very
different characters -- and one of them is a package we have to handle with care.

::: {.callout-note}
## What `symlim` is doing, and why it is staged

Julia's symbolic limits come from `SymbolicLimits`, which implements the *Gruntz algorithm*
for log-exponential functions **at infinity**. It is very good at that. Away from it,
as of v1.2.0, it does not cancel common factors, so `limit((x^2-1)/(x-1), x, 1)` returns
`0` rather than `2` -- and reports its assumptions as satisfied, so nothing warns you.

`symlim` therefore tries the dependable routes first and asks that engine last:

| route | what it did |
|:--|:--|
| `:substitution` | the expression was defined at $c$; evaluate it |
| `:cancel` | a removable singularity; cancel the common factor, then evaluate |
| `:series` | expand numerator and denominator as Taylor series, compare leading orders |
| `:reciprocal` | at infinity, a ratio of polynomials; substitute $x \to 1/u$ and cancel |
| `:gruntz` | hand it to `SymbolicLimits` -- for limits at infinity |
| `:divergent_numeric` | the values grow without bound |
| `:squeeze` | bound the function by intervals that collapse to a point |
| `:sides_disagree` | the left and right limits both exist and differ -- so there is none |
| `:undefined_on_side` | a one-sided limit was asked for on a side outside the domain |
| `:unresolved` | no method applied; **no answer is invented** |

The first four are exact and give rationals. Every route's answer is then checked against
what the function actually does numerically on the side being approached; an answer that
contradicts the evidence is discarded and the next route tried. Pass `side = :left` or
`side = :right` for a one-sided limit.
:::
...
We look again at this function which despite having a vertical asymptote at $x=15/11$ has
the property that it is positive for all floating point values, making both a numeric and
graphical approach impossible:
...
We find the limit at $c=15/11$ as follows, taking care to use the exact value `15//11` and
not the *floating point* approximation returned by `15/11`:
...
The limit is $-\infty$, as the asymptote demands. The route is worth reading: the answer
came from watching the values grow rather than from algebra. That is not a shortcut -- it is
the only route available. The absolute value blocks the series expansion, and `SymbolicLimits`
declines `abs` outright. What makes the numeric evidence trustworthy *here*, when the earlier
table was so misleading, is that we are no longer asking it for a finite value to five
decimal places; we are asking whether the values settle at all, and they plainly do not.
...
```{julia}
symlim((2sin(x) - sin(2x)) / (x - sin(x)), x, 0)
```

exactly $6$. The second is similarly done, though here we define a function for variety:
...
The third involves a parameter, and needs a rewrite first. Taken as written it returns
nothing at all:


```{julia}
@variables rho::Real
symlim((x^(1-rho) - 1) / (1 - rho), rho, 1)
```

A power with a symbolic exponent is outside every route: it is not a rational function to
cancel, and $x^{1-\rho}$ has no series in $\rho$. But $x^{1-\rho}$ is only another way of
writing $e^{(1-\rho)\log x}$, and *that* is a log-exponential expression -- precisely the
form the Gruntz engine was built for:


```{julia}
symlim((exp((1 - rho) * log(x)) - 1) / (1 - rho), rho, 1)
```

giving $\log(x)$ -- by the `:series` route, as it happens, since once the power is written
as an exponential the expression has a Taylor series in $\rho$ and the series stage reaches
it before the engine is ever asked. Rewriting an expression into a form some method can
handle is a normal part of symbolic work, not a workaround; which method then takes it is
worth reading off the route rather than assuming.


##### Example: exact values and where exactness is lost


In the example above we used `15//11` and not `15/11`. The distinction is worth dwelling on,
because it is a place where a correct calculation can silently answer a different question
than the one intended.


`15//11` is a `Rational`: exact. `15/11` is a `Float64`: the nearest representable number to
$15/11$, which is not $15/11$. Substituting each into $11x - 15$ shows the difference:


```{julia}
substitute(11x - 15, Dict(x => 15 // 11)), substitute(11x - 15, Dict(x => 15 / 11))
```

The first is exactly zero. The second is not -- and at that point the asymptote we were
studying is no longer at the point we are asking about.


The same trap is set by $\pi$. Consider $j(x) = \cos(x)/(x - \pi/2)$ near $\pi/2$:
...
The value is `Inf`, not `NaN`, because `cos(pi/2)` is not exactly $0$: `pi/2` is a rounded
floating point number. In `Symbolics`, `pi` on its own *is* kept exact -- but only until
Julia does arithmetic on it:


```{julia}
substitute(cos(x), Dict(x => Num(pi)/2)), substitute(cos(x), Dict(x => pi/2))
```

`Num(pi)/2` stays symbolic as $\pi/2$; plain `pi/2` was divided in floating point *before*
`Symbolics` ever saw it, and arrives as `1.5707963267948966`. The rule that follows is
simple: **build exact values with `//` and `Num(pi)`, and never let Julia evaluate a
constant expression first.**


This particular limit is one we must take numerically. Expanding a series about $\pi/2$
requires `Symbolics.taylor` to place $\pi$ in the coefficients, and it converts it to a
float; the expansion is then contaminated from its constant term onward. A graph suggests
the answer:
...
and the numeric table confirms it, converging on $-1$ from both sides:
...
Right and left limits will be discussed in the next section; here we give an example of the
idea. The mathematical convention is to say a limit exists if both the left *and* right
limits exist and are equal. Informally a right (left) limit at $c$ only considers values of
$x$ more (less) than $c$.


The ceiling function shows a case where they differ. Approaching $0$ from the right, every
value of $\lceil x \rceil$ is $1$; from the left, every value is $0$:
...
The two disagree, so the limit does not exist at $0$ -- though each one-sided limit exists
perfectly well. `symlim` reports directional limits through `SymbolicLimits` where it can,
but the ceiling function is not one of the cases it implements, so the numeric tables above
are what settle it.
...
converging on $\pi/4$. Putting together, we would get $1/2$. Which we could have done
directly in this case:


```{julia}
lim(x -> sin(pi*x)/(pi*x) * l(x), 1/2)
```

Both are taken numerically for the reason given earlier: an expansion about $1/2$ puts
$\pi$ into the series coefficients, where it is converted to a floating point value, so the
symbolic route is not available here.
...
# 1/18 exactly: tlim((x*sin(sin(x)) - sin(x)^2), x^6, x) -- the numerator's series is
# (1//18)x^6 - (1//45)x^8, so it vanishes to exactly the denominator's order. A numeric
# estimate is worthless here: dividing by x^6 at h=1e-4 means dividing by 1e-24, and the
# subtractive cancellation returns 0.0.
val = 1/18
numericq(val)
```
...
val = 11   # tlim(tan(x) - 24tan(x/2), 4sin(x) - 5x, x) gives 11//1 exactly
numericq(val)
```
...
#| echo: false
#| output: false
# display (1 + k x) the way the (1 + u)^(1/u) limit is written; reset after the next cell
set_conventional_default(; order = :ascending)
```

```{julia}
#| hold: true
@variables m::Real k::Real
ex = (1 + k*x)^(m/x)
```
...
What is $\lim_{x \to 0}$ of this expression?

(This one is beyond the tooling: `symlim` returns `:unresolved`. A power with a symbolic
exponent has no series in `x`, and `SymbolicLimits` does not terminate on this family --
including on the `exp`/`log` rewrite. It is the $(1+u)^{1/u} \to e$ limit in disguise, so
reason it out rather than computing it.)
...
What is the limit at $0$ of `ex`, defined here:
...
Taken as written, `symlim(ex, x, 0)` is `:unresolved` -- `a^x` has a symbolic exponent. Written
as $e^{x\log a}$ it is a log-exponential expression, and the limit follows:


```{julia}
symlim((exp(x*log(a)) - 1)/x, x, 0)
```
...
That answer, $\log(a)$, quietly assumes something about $a$ -- and nothing in the code says
so. `@variables a::Real` fixes a *type*, not a domain: there is no way to tell `Symbolics`
that $a$ is positive, and it will hand back $\log(a)$ whatever $a$ may be.

Is the restriction $a > 0$ needed for this limit to mean anything?
...
It is, on three counts. For $a^x$ to be real at every $x$ near $0$ we need $a > 0$: a
negative base has no real value at fractional exponents -- $(-8)^{1/2}$ is not real -- so
$a^x$ is not a real function on any interval around $0$, and there is nothing for a limit to
approach. At $a = 0$ it fails differently: $0^x$ is $0$ for positive $x$ and undefined for
$x \le 0$, so no two-sided limit exists. And $\log(a)$, the answer itself, is only defined
for $a > 0$.

So $a > 0$ is not a technicality, but the condition under which the question means
anything at all -- and here it is carried entirely by the reader, not by the tool.
...
The argument in the graph can be *computed*, and this is one of the few places where a
limit is established by bounding rather than by algebra. Interval arithmetic evaluates an
expression over a whole interval at once and returns an enclosure: a range guaranteed to
contain every value the function takes there. Squeeze the interval towards $0$ and watch:

```{julia}
#| hold: true
f(x) = x * sin(1/x)
for h in (1//10, 1//100, 1//1000)
    box = IntervalArithmetic.interval(-float(h), float(h))
    println(rpad("x ∈ ±$h", 14), f(box))
end
```

Each enclosure traps every value of $f$ near $0$, and their widths collapse. That *is* the
squeeze theorem -- $g$ and $h$ are the two ends of the box -- carried out as a computation
rather than read off a picture. `symlim` runs the same argument, and names it:

```{julia}
symlim(x * sin(1/x), x, 0)
```

The route matters as much as the value here. `:squeeze` says the answer was reached by
bounding, not by cancelling or by a series -- which is the honest description, since
$x\sin(1/x)$ has no useful series at $0$.

Note what the method does *not* claim. Applied to $\sin(x)$ as $x \to \infty$ the
enclosure is $[-1, 1]$ at every scale and never collapses, so `symlim` declines rather
than inventing a value -- correctly, as that limit does not exist:
...
```{julia}
@variables theta::Real r::Real
```

```{julia}
#| hold: true
x = r * tan(theta/2)
n = 2*Num(pi)/theta   # Num(pi) keeps pi exact; plain 2pi/theta would float it
# C < n * 2x
upper = n*2x
```
