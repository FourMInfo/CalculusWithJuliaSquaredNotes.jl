...
`CalculusWithJuliaSquared` re-exports `Roots`, `Plots` and `Symbolics`, so none of the
three needs a `using` of its own. It also teaches `Roots` to accept a symbolic expression
or an equation wherever it expects a function, which is what lets `find_zero(cos(x) ~ x,
(0, 2))` below work as written. Where the text asserts something about a limit — that a
function jumps, that it blows up at an endpoint, that a value is indeterminate in floating
point but not in mathematics — we compute it with `symlim` rather than assert it, and the
*route* it reports is the evidence.
...
The `NaN` is a fact about floating point, not about the function. Mathematically the limit
exists, and we need not take that on faith:


```{julia}
#| hold: true
@variables x
symlim(exp(x) - x^4, x, Inf)
```

The `:gruntz` route ranks $e^x$ against $x^4$ and reports $\infty$: the exponential wins,
so the indeterminate $\infty - \infty$ resolves. Evaluating `q(Inf)` asks floating point
to subtract one infinity from another, which it cannot do; taking the limit asks the
...
Well, we haven't even seen the peak yet. Better to do a little spade work first. This is a quadratic function, so we can solve for its roots exactly with `symbolic_solve`.


One step of preparation pays off here. With $\theta = \pi/4$ we have $\tan\theta = 1$ and
$v_0\cos\theta = 100\sqrt{2}$, so the model collapses to

$$
j(x) = x - \frac{32}{2}\cdot\frac{x^2}{20000} = x - \frac{x^2}{1250},
$$

which is the same function with its coefficients written exactly rather than as floating
point values:


```{julia}
@variables x
symbolic_solve(x - x^2/1250 ~ 0, x)
```

We see that $1250$ is the largest root. It is worth pausing on why we rewrote the model
first. Passing `j(x)` straight in also works, but `tan(pi/4)` is `0.9999999999999999` in
floating point, and `symbolic_solve` is exact: handed those coefficients it faithfully
returns the *exact rational* that the inexact input describes, a nineteen-digit fraction
that is $1249.9999999999998$ and not $1250$. The engine is not wrong; the input was never
quite the model. Doing the arithmetic by hand first is the difference between an answer
about our function and an answer about its floating point shadow.


So we plot over this domain to visualize the flight:
...
That the two one-sided limits disagree is exactly what `symlim` reports, and it reports it
by *refusing* the two-sided limit rather than by returning a number:
...
The route `:sides_disagree` is the diagnosis. The two sides individually are perfectly
well defined, and they are what the algorithm crossed between:


```{julia}
#| hold: true
@variables x
symlim(1/x, x, 0; side = :left), symlim(1/x, x, 0; side = :right)
```

So the sign change the bisection method detected is real, and the point it converged on is
real, but it is a pole rather than a zero. The algorithm cannot tell those apart, because
it only ever looks at signs. We can.
...
The asymptotes are the reason, and they can be computed rather than asserted. Note that
the limit point is written `Num(pi)/2`, which keeps $\pi/2$ exact instead of handing over
a float that merely rounds to it:


```{julia}
#| hold: true
@variables x
symlim(tan(x), x, Num(pi)/2; side = :left), symlim(tan(x), x, Num(pi)/2; side = :right)
```

Approaching $\pi/2$ from the left the function runs off to $+\infty$; from the right it
comes back from $-\infty$. Neither one-sided limit is a value, so there is nothing at the
endpoint for a maximum or minimum to *be*. That is precisely the hypothesis the extreme
value theorem needs and does not have here.
...
The claim that this function is continuous at the cusp is one the tooling can settle. A
cusp is a failure of *differentiability*, not of continuity, and the two are easy to
conflate when looking at a sharp corner on a graph:
...
The `:substitution` route *is* the definition of continuity at a point, computed: the
value at $0$ exists and the approach agrees with it. The graph has a corner there, and the
function is continuous there anyway, which is why the extreme value theorem still applies
...
(From [Strang](https://ocw.mit.edu/courses/res-18-001-calculus-fall-2023/resources/mitres_18_001_f17_ch00_pdf/), p. 48)
...
caption = """
Trajectories of potential cannonball fires with air-resistance included.
"""
