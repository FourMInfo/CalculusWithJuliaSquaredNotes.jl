...
`Symbolics` has no trig expander -- there is no `expand_trig`, and `simplify` will not
rewrite $\sin(x+h)$ -- so we apply the angle-addition formula
$\sin(x+h) = \sin(x)\cos(h) + \sin(h)\cos(x)$ by hand:
...
We can at least spot-check that rearrangement numerically:


```{julia}
a, b = 1.23, 0.45        # sample values for x and h
sin(a + b) - sin(a), cos(a)*sin(b) - sin(a)*(1 - cos(b))
```
...
and take a limit. `Symbolics` has no finite `limit`, so in place of a symbolic answer we
gather numeric evidence, in two steps. First, watch the difference quotient converge at a
sample value of $x$:
...
We verify numerically, the same way -- first the convergence at a sample $x$:
