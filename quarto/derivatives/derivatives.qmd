...
::: {.callout-warning}
## Cancel first, then take the limit

Handing `SymbolicLimits` the *uncancelled* quotient `((x+h)^5 - x^5)/h` returns `0` -- no
error, just a wrong answer. The `simplify(expand(...))` step above is not tidying: it is
what makes the limit correct. When a difference quotient is still in $0/0$ form, cancel
the $h$ before taking any limit, and sanity-check the result numerically.
:::
