...
gets symbolic limits from a companion package, `SymbolicLimits`, whose `limit` wants
unwrapped expressions and returns a `(value, assumptions)` pair -- so we wrap it once for
convenience:


```{julia}
symlimit(ex, v, c) = SymbolicLimits.limit(Symbolics.unwrap(ex), Symbolics.unwrap(v), c)[1]
...
Had we gone straight to the limit of the difference quotient in its raw $0/0$ form --
`symlimit(((x+h)^5 - x^5)/h, h, 0)` -- `SymbolicLimits` would have answered `0`: no error,
...
So: expand and cancel a $0/0$ quotient before taking its limit, and check a new result
numerically the first time. Happily, the algebra the tooling needs is the same binomial
expansion we would do by hand anyway.
:::
...
Those two limits are exactly where the symbolic tooling stops: `SymbolicLimits` is a
log-exponential engine and does not implement trigonometric functions at all. So here --
unlike the power rule above -- we gather numeric evidence, in two steps. First, watch the
difference quotient converge at a sample value of $x$:
...
Exponentials are also within reach of `SymbolicLimits`, so this derivative can be confirmed
directly -- and, like the logarithm, straight from the difference quotient:


```{julia}
symlimit((exp(x + h) - exp(x))/h, h, 0)
```
