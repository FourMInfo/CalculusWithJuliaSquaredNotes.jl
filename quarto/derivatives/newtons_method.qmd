...
For the function $f(x) = \cos(x) - x$, we see that Symbolics can not solve symbolically for a zero — `symbolic_solve` returns `nothing`, so no solution is displayed below:


```{julia}
@variables x
symbolic_solve(cos(x) - x, x)
```
