...
Using automatic derivatives, as brought in with the `CalculusWithJuliaSquared` package, we can implement this algorithm.
...
For the function $f(x) = \cos(x) - x$, we see that Symbolics can not solve symbolically for a zero — `symbolic_solve` returns `nothing`:


```{julia}
@variables x
@show symbolic_solve(cos(x) - x, x)
```
