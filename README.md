...
**The built book contains only the chapters ported so far.** `quarto/_quarto.yml` lists the published set; the rest of the upstream chapter list is kept directly beneath it, commented out, and a chapter is published by moving its line up as it lands. The landing page and sidebar therefore show exactly what exists — earlier, while every unported chapter was still listed, the generated index redirected to itself and looping was the only thing a browser could do.

Two alternatives: `quarto preview` from `quarto/` serves *and* rebuilds on edit (best while actively writing a chapter, but it re-executes cells, which is slow for heavy chapters); or, without re-executing anything, `julia --project=@liveserver -e 'using LiveServer; serve(dir="quarto/_book", port=8001)'` from the repo root (the shared `@liveserver` environment). Not `python3 -m http.server`: this fork exists to remove Python.
